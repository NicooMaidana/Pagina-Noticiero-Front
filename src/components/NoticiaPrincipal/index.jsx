...
const NoticiaPrincipal = ({ ...props }) => {
  ...
  const imagenNoticiaPrincipal = {
    backgroundImage: `url(${props.imagen})`,
  }

  return (
    <>
      <div className={`flex justify-center items-center md:px-0 lg:px-0 sm:px-5 ${props.className}`}>
        <div
          className="containerNoticiaPrincipal bg-white rounded-xl shadow-lg transform w-full"
          id="noticiaPrincipal"
          style={imagenNoticiaPrincipal}
        >
          ...
          <div className="contenidoNoticiaPrincipal bottom-0 right-0 h-full rounded-b-xl text-white p-5 pt-3  duration-300 cursor-pointer">
            <div className="w-full h-1/3 mb-4 ">
              <h2 className=" mt-0 mb-2  cursor-pointer text-white text-Left">
                {props.titulo}
